...
let iconFile; 
window.addEventListener("load", () => {     //to load the application
  let long; //longitude
  let lat;  //latitude

  if (navigator.geolocation) { //current location
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      fetch(api)
        ...
        .then((response) => {
          return response.json(); //got the respose and converting it in json
        })
        .then((data) => {
          console.log(data);
          const { name } = data;
